# Acknowledgement: The code in the functions create_X_tensor(), create_y_train(), and create_y_tensor()
# was adapted from code written by Gordeev and Singer for their model
...
def create_y_train(play_track_df, min_idx_y, max_idx_y):
    ...
    # Add 99 yards to ceate index so we never have negative yards
    train_y['YardIndex'] = train_y["kickReturnYardage"].apply(lambda val: val + 99)
    # Clip yards index so that all indices fall within min_idx_y and max_idx_y
...
if __name__ == '__main__':
    ...
    get_input_data_combo(kick_play_track_df, punt_play_track_df, "comb", 94, 154,test_size=0.1)
    get_input_data(kick_play_track_df, "kick", 94, 154, test_size=0.1, save_tensors=True)
    get_input_data(punt_play_track_df, "punt", 94, 154, test_size=0.1, save_tensors=True)
