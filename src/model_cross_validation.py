...
decay_steps_dict = {"comb_acc":50000, "comb_elo":50000, "comb_force_momentum":100000, "comb_momentum":50000, "comb":50000,
                    "kick_acc":50000, "kick_elo":100000, "kick_force_momentum":50000, "kick_momentum":50000, "kick":50000,
                    "punt_acc":100000, "punt_elo":100000, "punt_force_momentum":50000, "punt_momentum":50000, "punt":100000,
                    "comb_contact":50000}
...
def plot_loss(history, i, data=''):
    ...
    Inputs:
        ...
        data: (str) Description of which input data type the model is training on
    '''
    plt.title(f"Learning Curve {data}")
    plt.xlabel('Epoch')
    plt.ylabel('CRPS')
    plt.plot(history.history['loss'], label='train')
    plt.plot(history.history['val_loss'], label='val')
    plt.legend()
    plt.savefig(f"model_plots/model_cross_validation/{data}/plot_{data}_{i}.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    decay_steps = decay_steps_dict[args.input_desc]
    ...
    # Determine which play type we're dealing with 
    if args.input_desc == "comb_contact":
        data_desc = "comb_contact"
    else:
    ...
    with open(f'model_cross_val_splits/{data_desc}_train_splits.data', 'rb') as f:
        train_splits = pickle.load(f)
    with open(f'model_cross_val_splits/{data_desc}_val_splits.data', 'rb') as f:
    ...
    Path(f"model_plots/model_cross_validation/{args.input_desc}").mkdir(parents=True, exist_ok=True)
    Path(f"model_cross_validation/{args.input_desc}").mkdir(parents=True, exist_ok=True)

    # Perform chunk of cross validation
    val_loss = []
    for i in split_indices:
        ...
        model, history = run_model(X_train_tensor, y_train_tensor, args.epochs, batch_size, lr, (X_val, y_val), decay_steps,
                                early_stop=True, input_channels=input_channels, data=args.input_desc)
        val_loss.append(min(model.history.history['val_loss']))

        plot_loss(history, i, data=args.input_desc)


    with open(f'model_cross_validation/{args.input_desc}/model_cross_validation_{args.input_desc}_{args.chunk}.data', 'wb') as f:
