...
def add_momentum(play_track_df, players_df):
    '''
    Add a column for momentum to the processed dataframe that contained the data
    ...
    Inputs:
        ...
        players_df: dataframe containing player related information. We are interested
                    in weight
    Returns:
        play_track_momentum_df: dataframe with all data needed for the momentum model. Each row in the df
                                contains the tracking data for a single player on a given play
    '''
    lb_to_kg = 0.453592 # Conversion from lbs to kg
    play_track_momentum_df = pd.merge(play_track_df, players_df, how='inner')   
    play_track_momentum_df["p_x"] = play_track_momentum_df["v_x"] * lb_to_kg * play_track_momentum_df["weight"]
    play_track_momentum_df["p_y"] = play_track_momentum_df["v_y"] * lb_to_kg * play_track_momentum_df["weight"]
    
    return play_track_momentum_df


def create_X_tensor_momentum(play_track_df):
    '''
    Creates X input tensor for momentum model. 
    ...
    for name, group in grouped_df:
        [[returner_x, returner_y, returner_Px, returner_Py]] = group.loc[group.player_side=="returner",['x', 'y','p_x','p_y']].values
        ...
        for j, kick_team_id in enumerate(kick_team_ids):
            [kick_team_x, kick_team_y, kick_team_Px, kick_team_Py] = group.loc[kick_team_id,['x', 'y','p_x','p_y']].values
            ...
            train_x[i,j,:,:4] = group.loc[return_team_ids,['p_x','p_y','x', 'y']].values - np.array([kick_team_x, kick_team_y, kick_team_Px, kick_team_Py])
            train_x[i,j,:,-6:] = [kick_team_returner_Px, kick_team_returner_Py, kick_team_returner_x, kick_team_returner_y, kick_team_Px, kick_team_Py]
        i += 1
...
def get_input_data_momentum(play_track_df, output_tag, min_idx_y, max_idx_y, test_size=0.1, save_tensors=True):
    '''
    ...
    '''
    X_tensor = create_X_tensor_momentum(play_track_df)
...
def get_input_data_momentum_combo(kick_play_track_df, punt_play_track_df, output_tag, min_idx_y, max_idx_y, test_size=0.1):
    '''
    Create the X and y train and test tensors for either kickoffs or punts
    Inputs:
        play_track_df: dataframe with all data needed for the model. Each row in the df
    ...
    Returns: 
        X_train_tensor, X_test_tensor, y_train_tensor, y_test_tensor: Input tensors for model
    '''
    X_train_kick_tensor, X_test_kick_tensor, y_train_kick_tensor, y_test_kick_tensor = get_input_data_momentum(kick_play_track_df, "kick", min_idx_y, max_idx_y, test_size=test_size, save_tensors=False)
    X_train_punt_tensor, X_test_punt_tensor, y_train_punt_tensor, y_test_punt_tensor = get_input_data_momentum(punt_play_track_df, "punt", min_idx_y, max_idx_y, test_size=test_size, save_tensors=False)
...
if __name__ == '__main__':
    kick_play_track_momentum_df = add_momentum(kick_play_track_df, players_df)
    punt_play_track_momentum_df = add_momentum(punt_play_track_df, players_df)

    get_input_data_momentum_combo(kick_play_track_momentum_df, punt_play_track_momentum_df, "comb_momentum", 94, 154)
    get_input_data_momentum(kick_play_track_momentum_df, "kick_momentum", 94, 154, test_size=0.1, save_tensors=True)
    get_input_data_momentum(punt_play_track_momentum_df, "punt_momentum", 94, 154, test_size=0.1, save_tensors=True)
