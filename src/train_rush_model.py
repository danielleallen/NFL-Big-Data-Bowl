...
def run_model(X_tensor, y_tensor, n_epochs, batch_size, lr, checkpoint_path, plot=False, data=''):
    '''
    Creates a new model, trains it, and continues to save the model as it trains
    Inputs:
...
if __name__ == '__main__':
    # Load rush training data
    with open('input_tensors/X_tensor_rush.data', 'rb') as f:
        X_train_tensor = pickle.load(f)
    with open('input_tensors/y_tensor_rush.data', 'rb') as f:
        y_train_tensor = pickle.load(f).astype('float32')

    model = run_model(X_train_tensor, y_train_tensor, 30, 64, 1e-3, "model_checkpoint/rush.ckpt", plot=True, data='rush')
